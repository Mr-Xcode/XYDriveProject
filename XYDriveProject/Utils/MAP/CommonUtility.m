...
+(MAMultiPolyline *)multiPolylineForCoordinateString:(NSString *)coordinateString
{
    ...
    MAMultiPolyline *polyline = [MAMultiPolyline polylineWithCoordinates:coordinates count:count drawStyleIndexes:@[@0,@(count-1)]];
...
+ (CLLocationCoordinate2D)bd09Encrypt:(double)gcLat bdLon:(double)gcLon
{
    ...
    double z = sqrt(x * x + y * y) + 0.00002 * sin(y * M_PI);
    double theta = atan2(y, x) + 0.000003 * cos(x * M_PI);
    CLLocationCoordinate2D bdjPt;
    bdjPt.longitude = z * cos(theta) + 0.0065;
    bdjPt.latitude = z * sin(theta) + 0.006;
    return bdjPt;
}
...
+ (void)setMapViewVisibleMapRect:(MAMapRect)mapRect withInsets:(UIEdgeInsets)insets animated:(BOOL)animated forMapView:(MAMapView *)mapView
{
    ...
    DLog(@"地图缩放范围x:%lf y:%lf width:%lf hwight:%lf",mapRect.origin.x,mapRect.origin.y,mapRect.size.width,mapRect.size.height);
